import frPracticeEn from './fr-practice-en'
import pensukeHelpEn from './pensuke-help-en'
import pensukePracticeEn from './pensuke-practice-en'

export default {
  title: 'Fewest Moves',
  header: {
    signIn: 'Sign in',
    signOut: 'Sign out',
  },
  colorMode: {
    system: 'System',
    light: 'Light',
    dark: 'Dark',
  },
  timezone: {
    title: 'Timezone',
    auto: 'Local time',
    competition: 'Competition time (UTC+8)',
    utc: 'UTC',
  },
  countdown: {
    days: 'Days',
    hours: 'Hours',
    minutes: 'Mins',
    seconds: 'Secs',
  },
  common: {
    id: 'ID',
    wcaId: 'WCA ID',
    email: 'Email',
    home: 'Home',
    more: 'More',
    status: 'Status',
    yes: 'Yes',
    createdAt: 'Created At',
    createdBy: 'Created By',
    timeUp: 'Time\'s up!',
    signingInRequired: 'Sign in Required',
    signingToJoin: 'Sign in to join',
    signingIn: 'Signing in with WCA ...',
    notation: 'notation',
    notationURL: 'https://www.worldcubeassociation.org/regulations/#12a',
    resultTitle: '{t} Result',
    moves: '{moves} moves',
    all: 'All',
    basicRules: 'Basic Rules',
    filterBy: {
      label: 'Filter By',
    },
    sortBy: {
      label: 'Sort By',
      moves: 'Moves',
      sumittedAt: 'Submitted At',
      fewest: 'Fewest',
      latest: 'Latest',
      mostContinuations: 'Most Continuations',
    },
    spoiler: 'Spoiler for {for}. Click to see.',
    backTo: 'Back to {to}',
    insertions: {
      mode: {
        normal: 'Normal',
        inverse: 'Inverse',
        both: 'Both',
      },
    },
    total: 'Total',
    result: 'Result',
    new: 'New',
    format: 'Format',
    bo1: 'Best of 1',
    bo2: 'Best of 2',
    mo3: 'Mean of 3',
    saveAsImage: 'Save as Image',
    expand: 'Expand',
    collapse: 'Collapse',
  },
  error: {
    400: 'Bad Request',
    403: 'Forbidden',
    404: 'Page not found',
    other: 'An error occurred',
    league: {
      seasonNotFound: 'Season not found',
    },
    wca: {
      competitionNotFound: 'Competition not found',
    },
  },
  loading: 'Loading...',
  optional: 'optional',
  description: {
    if: 'This is insertion finder.',
    sf: 'This is slicey finder.',
  },
  index: {
    defination: '{title} (or {or} {fmc}) is an event where competitors attempt to solve a puzzle (almost always the 3x3x3) in as few moves as possible, starting from a given scramble. {wiki}',
    speedsolvingWiki: 'SpeedSolving.com Wiki',
    latest: 'Latest Findings',
  },
  sf: {
    title: 'Slicey Finder',
    shortTitle: 'SF',
    description: 'It\'s a useful tool to check the slicey insertions for 3x3x3 Fewest Moves.',
  },
  if: {
    title: 'Insertion Finder',
    shortTitle: 'IF',
    description: 'It\'s a useful tool to check the optimal insertions for 3x3x3 Fewest Moves.',
    latest: 'Latest IF',
    find: 'Find Again',
    name: {
      label: 'Name',
      description: 'You can type something to identify this insertion.',
    },
    scramble: {
      label: 'Scramble',
      description: 'Please don\'t input any invalid {notation}.',
      invalid: 'Please check your scramble!',
    },
    skeleton: {
      label: 'Skeleton',
      description: 'Only the followings are allowed:',
      invalid: 'Please check your skeleton!',
      list: [
        'WCA {notation}.',
        '"()" or "^()" for inverse moves. "NISS" for switching scramble. But "()" and "NISS" can\'t be used at the same time.',
        'Any content after "//" in each line will be considered as comments.',
      ],
      to: '{length} to {detail}',
      solved: '{length} to solved',
    },
    algs: {
      'label': 'Algorithms',
      'description': 'Please choose at least one algorithm.',
      'corner': {
        label: 'Corners',
      },
      'edge': {
        label: 'Edges',
      },
      'other': {
        label: 'Other',
      },
      'extra': {
        label: 'Extra',
      },
      '3CP': {
        label: '3 Corners',
        description: '',
      },
      '3CP-pure': {
        label: 'Pure 3 Corners',
        description: '',
      },
      '2x2CP': {
        label: '2-2 Corners',
        description: '',
      },
      'CO': {
        label: 'Corner Twists',
        description: '',
      },
      'C-other': {
        label: 'Other Corners',
        description: '',
      },
      '3EP': {
        label: '3 Edges',
        description: '',
      },
      '2x2EP': {
        label: '2-2 Edges',
        description: '',
      },
      'EO': {
        label: 'Edge Flips',
        description: '',
      },
      '3CP3EP': {
        label: '3 Corners and 3 Edges',
        description: '',
      },
      'E-other': {
        label: 'Other Edges',
        description: '',
      },
      'parity': {
        label: 'Parity',
        description: '',
      },
      'center': {
        label: 'Center',
        description: '',
      },
      'no-parity-other': {
        label: 'Other Algs without Parity',
        description: '',
      },
      'extras/parity': {
        label: 'Extra Parity',
      },
      'extras/no-parity-other': {
        label: 'Extra Algs without Parity',
      },
      'all': 'All',
      'none': 'None',
      'necessary': 'Necessary',
    },
    greedy: {
      label: 'Greedy',
      description: 'It\'s a number N. Let\'s say when searching insertions for each step, the minimum moves of all skeletons is M. IF will keep all skeletons whose moves equal or less than M + N moves. 0 means always best first (human mode?).',
    },
    cycles: {
      label: 'Cycles',
      corners: 'Corners',
      edges: 'Edges',
      centers: 'Centers',
      parity: 'Parity',
    },
    status: [
      'Waiting',
      'Computing',
      'Finished',
    ],
    solutions: {
      label: 'Solutions',
      insertion: 'Insertion',
      insertions: 'Insertions',
      final: 'Final Solution',
      cancellation: 'Cancellation',
      exceed: 'There\'re too many insertions to be inserted. I don\'t want to do it.',
      noProper: 'There\'s no proper insertion.',
      merged: 'Merged',
      expanded: 'Expanded',
    },
    fewestmoves: 'Fewest Moves',
    duration: 'Calculation Duration',
  },
  user: {
    profile: 'My Profile',
    if: 'My IF',
    likes: 'Likes',
    favorites: 'Favorites',
    name: 'Name',
    avatar: 'Avatar',
    changeName: 'Change Name',
    solutions: 'My Solutions',
    practices: 'My Practices',
    token: 'Bot Token',
  },
  admin: {
    title: 'Admin',
    user: {
      title: 'User',
      if: '{name}\'s IF',
      ifs: 'Finder Count',
    },
    if: {
      title: 'IF',
    },
    tutorial: {
      title: 'Tutorial Management',
      add: 'Add Tutorial',
      edit: 'Edit Tutorial',
      unsaved: 'Sort order changed. Save to apply.',
      form: {
        title: 'Title',
        url: 'URL',
        category: 'Category',
        language: 'Language',
      },
    },
  },
  form: {
    submit: 'Submit',
    reset: 'Reset',
    save: 'Save',
    saved: 'Saved',
    confirm: 'Confirm',
    cancel: 'Cancel',
    remove: 'Remove',
    removeConfirm: 'Are you sure to remove this?',
    edit: 'Edit',
    delete: 'Delete',
    decline: 'Decline',
    incline: 'Incline',
    working: {
      label: 'Working Paper',
      description: 'You can upload your working papers in image format.',
    },
  },
  weekly: {
    title: 'Weekly Competition',
    shortTitle: 'Weekly',
    scrambles: 'Scramble | Scrambles',
    scramble: 'Scramble {number}',
    period: {
      started: 'Started at {start}.',
      pendingStart: 'Will start at {start}.',
      onGoing: 'Started at {start} and will end at {end}.',
      ended: 'Ended at {end}.',
      startsIn: 'Starts in',
      endsIn: 'Ends in',
    },
    join: 'Join Now',
    mode: {
      label: 'Mode',
    },
    regular: {
      label: 'Regular',
      description: 'Regular FMC rules, 1 hour limit.',
      unlimitedSubmitted: 'You have already submitted solution for unlimited mode. The result of regular mode will be DNFed.',
    },
    unlimited: {
      label: 'Unlimited',
      description: 'No time limit. You can submit many times.',
      invalid: 'Please submit better solution!',
    },
    solution: {
      label: 'Solution',
      description: 'Please don\'t input any invalid {notation}.',
      invalid: 'Please check your solution!',
    },
    comment: {
      label: 'Comment',
      description: 'You can type something to describe your solution.',
    },
    confirmDNF: 'Are you sure to DNF?',
    submitted: ' (Submitted)',
    solutions: 'Solutions',
    noSolution: 'No Solution',
    seeSolutions: 'Submit your solution to see {solutions} result | Submit your solution to see {solutions} solutions',
    results: 'Results',
    unlimitedResults: 'Results (Unlimited)',
    past: 'Past Competitions',
    turnToUnlimited: {
      label: 'Turn to Unlimited',
      confirm: 'Are you sure to turn this result to unlimited mode? This is irreversible. After that, you regular result will be DNFed and you CANNOT submit solution for regular mode on this attempt.',
    },
    rules: {
      basic: {
        title: 'Basic Rules',
        rules: [
          'You can only use the {notation} that is defined in WCA regulations.',
          'For regular mode, you should finish the solve within 1 hour. Result must be submitted before submitting in unlimited mode.',
          'For unlimited mode, there\'s no time limit. You can submit many times. But only better result can be submitted.',
          'Any kind of cheating is not allowed.',
          'Comment should be submitted too. You can change it after submitting though.',
        ],
      },
    },
    previous: 'Previous',
    next: 'Next',
  },
  daily: {
    title: 'Daily Competition',
    shortTitle: 'Daily',
    past: 'Past Competitions',
    pending: 'Pending Creation',
    creation: 'Every first BO1 practice created will be the daily competition. The creator should submit valid solution on his latest practice solution.',
  },
  funChallenge: {
    title: 'Fun Challenges',
    shortTitle: 'Fun',
    past: 'Past Challenges',
    pending: 'No fun challenge is running.',
    type: 'Type',
    rules: 'Submit a full solve. This challenge uses its selected metric for ranking.',
    centerRestored: 'Centers restored',
    centerNotRestored: 'Centers are not restored',
    metricMoves: '{metric}: {moves}',
    metricMovesWithHtm: '{metric}: {moves}, HTM: {htm}',
    types: {
      8: 'QTM Challenge',
      9: 'STM Challenge',
      10: 'ATM Challenge',
      11: 'Center Solved Challenge',
    },
    admin: {
      title: 'Fun Challenges Admin',
      shortTitle: 'Fun Admin',
      startTime: 'Start Time',
      endTime: 'End Time',
      scrambles: 'Scrambles',
      scramblesDescription: 'Optional. Enter one scramble per line. Leave empty to generate automatically.',
      recent: 'Recent Challenges',
      start: 'Start',
      end: 'End',
      delete: 'Delete',
      deleteConfirm: 'Delete {name}?',
    },
  },
  tutorial: {
    title: 'Tutorial',
    resources: {
      description: 'A curated collection of FMC tutorials and resources.',
      empty: 'No tutorials yet.',
    },
    htrDiagram: {
      title: 'HTR Routine & Maze',
      description: 'The amazing HTR {picture} is created by {author}.',
      author: {
        peng: '2013PENG02',
        jian: '2011JIAN07',
      },
      picture: {
        routine: 'Routine',
        maze: 'Maze',
      },
    },
  },
  endless: {
    title: 'Endless Challenge',
    shortTitle: 'Endless',
    description: 'Endless Challenge is a new event that you can challenge yourself to pass through levels as many as possible. It\'s a pre-alpha version. The rules may change in the future.',
    rules: [
      'Only valid solutions can be submitted.',
      'At the beginning, all people will start from level 1.',
      'New level will open once one of the following conditions is satisfied:',
      'Each level\'s scramble is visible after you enter the level.',
      'Each level\'s solutions are visible after you submit a valid result.',
      'Any kind of cheating is not allowed.',
    ],
    challenge: {
      any: 'A person submits a valid solution.',
      single: 'A person gets a result that is less than or equal to {moves} moves.',
      team: '{persons} persons get results that are less than or equal to {moves} moves.',
      bossInstantKill: 'A result of {moves} moves or less defeats the boss instantly.',
      bossHitPoints: 'Boss HP ranges from {min} to {max}.',
    },
    level: 'LV {level}',
    progress: {
      title: 'Progress',
      competitors: '{competitors} People',
    },
    continue: 'Continue',
    previous: 'Previous Level',
    next: 'Next Level',
    toBeContinued: 'To be Continued',
    kickedBy: 'Kicked by',
    kickCondition: 'Next Level: A result that ≤{single} moves or {persons} results that ≤{team} moves submitted.',
    kickConditionAny: 'Next Level: a valid solution is submitted.',
    bossKickCondition: 'Next Level: A result of ≤{instantKill} moves defeats the boss instantly; otherwise submissions deal damage based on move count.',
    bossCurrentHp: 'Current Boss HP: {hp}',
    bossDnfPenalty: 'DNF penalty active: this level cannot instant-kill the boss, and all damage is halved.',
    instantKill: 'instant kill',
    damageDealt: 'damage {damage}',
    stats: {
      title: 'Statistics',
      singles: 'Best Singles',
      means: 'Best Means',
      averages: 'Best Averages',
      rollingMo3: 'Best Rolling Mo3',
      rollingAo5: 'Best Rolling Ao5',
      rollingAo12: 'Best Rolling Ao12',
      highestLevels: 'Highest Levels',
      personal: 'Personal Stats',
      moves: 'Moves',
      count: 'Count',
      level: 'Level',
    },
    openAt: 'Open at {time}.',
    kickedAt: 'Kicked at {time}.',
    mode: {
      description: 'This only marks the mode of the solution.',
    },
    unlimited: {
      description: 'No time limit. Try to get good solutions.',
    },
    type: [
      'Regular endless challenge with same requirements for every level.',
      'Every level is a boss level. Every 10th level has much higher HP and a stricter instant-kill threshold.',
      'The EO practice has EO done in every scramble.',
      'The DR practice has DR done in every scramble.',
      'The HTR practice has HTR done in every scramble.',
      'In the hidden scramble challenge, the scramble is hidden and only the scrambled state is given. It means you need to clone a scrambled cube by yourself.',
      'The JZP practice has JZP done in every scramble.',
      'Mystery mode: each level has hidden conditions that are revealed when triggered by submissions.',
    ],
    showAll: 'Show All',
    ended: 'The following challenges have ended. No new level will be added. You can submit solutions to the existing levels.',
  },
  endlessChallenge: {
    title: 'Endless Challenge - Conditions',
    description: 'Each level has hidden conditions. Submit solutions to trigger and reveal them. When all conditions are met, the next level opens.',
    autoRevealed: 'auto-revealed',
    rules: {
      title: 'Rules',
      submit: 'Submit a valid solution to each level.',
      conditions: 'Each level has hidden conditions (regular levels: 1-3, boss levels: 3-5).',
      reveal: 'When your submission meets a condition, it is revealed.',
      allMet: 'The next level opens when all conditions of the current level are met.',
      autoReveal: 'If no condition is triggered for 3 days, one condition is automatically revealed.',
      boss: 'Every 10th level (10, 20, 30...) is a boss level with more conditions.',
    },
    conditions: {
      title: 'Conditions',
      hidden: 'Hidden condition — submit to reveal',
      allMet: 'All conditions met! Next level unlocked!',
    },
    conditionType: {
      movesEqual: 'A solution is exactly {moves} moves',
      movesEqualMulti: '{count} solutions are exactly {moves} moves',
      movesLe: 'A solution is at most {moves} moves',
      movesLeMulti: '{count} solutions are at most {moves} moves',
      movesGe: 'A solution is at least {moves} moves',
      movesGeMulti: '{count} solutions are at least {moves} moves',
      movesParity: 'A solution has a move count that is {parity}',
      sameSolution: '{count} people submit the same solution',
      sameMoves: '{count} people submit solutions with the same move count',
      allDifferentMoves: 'At least {min} submissions, all with different move counts',
      totalSubmissions: 'At least {count} people submit a solution',
      consecutiveMoves: '{count} submissions have move counts in an arithmetic sequence (common difference {diff})',
    },
    parity: {
      'even': 'even',
      'odd': 'odd',
      'multiple-of-3': 'divisible by 3',
      'multiple-of-5': 'divisible by 5',
      'multiple-of-7': 'divisible by 7',
    },
  },
  result: {
    rank: 'Rank',
    name: 'Name',
    best: 'Best',
    mean: 'Mean',
    solves: 'Solves',
    single: 'Single',
    challenge: 'Challenge',
    competition: 'Competition',
    sequence: 'No.',
    competitionOrder: 'No. {n}',
    date: 'Date',
    bestAverages: 'Best averages',
    round: 'Round',
    roundType: {
      1: 'First',
      d: 'First',
      2: 'Second',
      e: 'Second',
      3: 'Semi Final',
      g: 'Semi Final',
      f: 'Final',
      c: 'Final',
    },
    week: 'Week',
    type: {
      weekly: 'Weekly',
      daily: 'Daily',
      league: 'League',
      practice: 'Practice',
      endless: 'Endless',
      wca: 'WCA',
    },
    moveCount: 'Move Count',
    chartOptions: {
      includeDNF: 'Include DNF',
      fillCounts: 'Fill empty counts',
      trim: 'Trim',
      timeRange: 'Time',
      attemptRange: 'Attempts',
      last3Months: 'Last 3 months',
      lastYear: 'Last year',
      recentAttempts: 'Last {count} attempts',
      attemptRangeLabel: 'Selected {selected} attempts (No. {start} - {end} of {total})',
      allRange: 'All',
    },
  },
  chain: {
    title: 'FMC Chain',
    shortTitle: 'Chain',
    description: 'FMC Chain is a new event that everyone can submit a partial solution to continue the previous one. You can submit in different phases like EO, DR, L3E, L3C3E, etc. A solution is chained by the previous one. Finally all solutions make a big tree for the given scramble.',
    rules: [
      'Only different solutions can be submitted.',
      'Each solution should let the cube be more closer to solved.',
      'In some phases like EO, DR and HTR, the last move must be clockwise. No meaningless half-turns are allowed.',
      'Inverse moves are allowed and must be marked with "()" or "^()". "NISS" is not allowed for not mixing with "()".',
    ],
    continue: 'Continue',
    continuances: '{n} Continuance | {n} Continuances',
    finishes: '{n} Finish | {n} Finishes',
    best: 'Best',
    tip: 'Click on the scramble or each phase to get back to the corresponding phase.',
    bestResults: 'Best Results',
    duplicate: 'This solution has been submitted by someone else.',
    status: {
      yet: 'Yet Started',
      declined: 'Declined',
      viewed: 'Viewed',
      latest: 'My Latest Submission',
      notification: 'New Submission',
    },
  },
  practice: {
    title: 'Practice Book',
    shortTitle: 'Practice',
    description: 'Description',
    descriptionPlaceholder: 'Add a description for this practice (supports Markdown)',
    new: {
      title: 'New Practice',
    },
    user: {
      title: '{name}\'s Practice Book',
      index: '{name}\'s Practice #{index}',
    },
    latest: 'Latest Practice',
    mostAttended: 'Most Attended',
    mostPractices: 'Most Practices',
    index: 'Practice #{index}',
    practices: '1 Practice | {n} Practices',
    previous: 'Previous',
    next: 'Next',
    created: 'Created Practices',
    joined: 'Joined Practices',
  },
  profile: {
    pr: 'Personal Records on 333.fm',
    noRecord: 'No Record',
  },
  bot: {
    token: {
      description: 'Bot Token is a one-time credential for binding user information on 333.fm with QQ groups.',
    },
  },
  league: {
    title: 'League',
    description: 'The competition is played in the form of a league, where each competitor plays every other competitor in the form of head-to-head matches (one per week).',
    bye: 'Bye',
    nav: {
      summary: 'Summary',
      tiers: 'Tiers',
      schedules: 'Schedules',
      standings: 'Standings',
      week: 'Week {week}',
      rules: 'Rules',
      statistics: 'Statistics',
      elos: 'ELO',
    },
    standing: {
      competitors: 'Competitors',
      sets: 'Sets',
      pts: 'PTS',
      wins: 'W',
      draws: 'D',
      losses: 'L',
      bestMo3: 'Best Mo3',
    },
    allResults: 'Results (all)',
    summary: {
      players: 'Players',
      weeks: 'Weeks',
      tiers: 'Tiers',
      ongoing: 'Ongoing - Week {week}',
      past: 'Past',
      upcoming: 'Upcoming',
    },
    mode: {
      participants: 'Participants',
      others: 'Others',
    },
    rules: {
      format: 'The competition is played in the form of a league, where each competitor plays every other competitor in the form of head-to-head matches (one per week).',
      sets: 'Every match is decided by sets. Getting a better result on a scramble means winning the set (1-0) and drawing on a scramble means drawing a set (0.5-0.5).',
      winner: 'The winner of a match is the player who gained more points from sets.',
      ranking: 'Players are ranked in the table based on the sum of the points from all duels, with 2 points for a win, 1 point for a draw and 0 points for a loss.',
      others: 'You aren\'t in the league. You can submit solutions for the scrambles without gaining points and see other results.',
      basic: [
        'You can only use the {notation} that is defined in WCA regulations.',
        'You should finish the solve within 1 hour.',
        'Any kind of cheating is not allowed.',
        'Don\'t post any spoilers about current week\'s scrambles until it\'s over.',
        'Comment should be submitted too. You can change it after submitting though.',
      ],
      criteria: 'Player classification criteria:',
      tierBreakers: [
        'Number of points',
        'Head-to-head matchup (if more than one player: small table for those players only)',
        'Number of wins',
        'Best single mo3 of the entire tournament, ',
        '2nd best mo3 and so on..',
      ],
    },
    participate: {
      description: 'If you aren\'t sure you will be able to commit to a full season, consider not signing up and treating league as a weekly. Dropping out mid season disturbs the process of rivalisation and collecting points within a tier.',
      label: 'Join next season',
      unparticipate: 'Quit next season',
      signUp: 'Season {number} Sign-Up',
    },
    statistics: {
      tier: 'Tier',
      avgRank: 'Avg Rank',
    },
    pastSeasons: 'Past Seasons',
    profile: {
      overview: 'Overview',
      currentElo: 'Current ELO',
      seasons: 'Seasons',
      totalDuels: 'Total Duels',
      totalWins: 'Wins',
      totalDraws: 'Draws',
      totalLosses: 'Losses',
      winRate: 'Win Rate',
      bestSeason: 'Best Season',
      duelHistory: 'Duel History',
      eloHistory: 'ELO History',
      seasonSummary: 'Season Summary',
      season: 'Season',
      tier: 'Tier',
      position: 'Position',
      points: 'Points',
      result: 'Result',
      opponent: 'Opponent',
      week: 'Week',
      score: 'Score',
      win: 'W',
      draw: 'D',
      loss: 'L',
      noData: 'No league data',
    },
  },
  tools: {
    title: 'Tools',
    drCaseRecognizer: {
      title: 'DR Recognizer',
      description: 'It\'s a useful tool to recognize the DR case for 3x3x3 Fewest Moves.',
    },
    frTrainer: {
      title: 'FR Trainer',
      fullName: 'FR = Floppy Reduction',
      subtitle: 'Reduce from HTR to R2 L2 F2 B2 — classify FR shapes on three axes and get reference solutions.',
      description: 'Practice Floppy Reduction (FR) from HTR state. Analyze cases, train interactively, or read the tutorial.',
      mode: {
        analyze: 'Analyze',
        practice: 'Practice',
        tutorial: 'Tutorial',
      },
      input: {
        scramble: {
          label: 'Scramble',
          placeholder: 'Scramble',
        },
        solution: {
          label: 'Solution',
          placeholder: 'Moves to HTR state, e.g. R2 U2 F2 L2 … (Cmd/Ctrl + Enter to analyze)',
        },
      },
      btn: {
        analyze: 'Analyze',
        random: 'Random',
        help: 'Reference',
      },
      leaveSliceMode: 'Leave slice mode',
      leaveSliceHint: 'Turn off to fully restore middle-layer edges (shortest FR solution, solvable by the four side half-turns)',
      falseFr: 'Fake FR',
      trueFr: 'FR',
      parityOff: 'Fake FR',
      inputFalseFrNote: 'The FR shape is met but the parity is off; one extra axis half-turn is needed to solve. The full FR solution is below.',
      falseFrSolution: 'Shortest shape solution (Fake FR)',
      correctSolution: 'Correct solution (FR)',
      steps: {
        toggle: 'Solution breakdown',
        start: 'Start',
        shapeTitle: 'Shape solution (Fake FR)',
        trueTitle: 'FR solution',
        frDone: 'FR ✓',
        falseFrDone: 'Fake FR (shape met but parity off)',
        triggerHint: 'Reached an FR trigger; finish with {alg}',
        clickHint: 'Click a step to preview that state on the cube above',
      },
      help: {
        title: 'Reference',
        close: 'Close',
        intro: 'Examples below use the U/D axis as the FR axis; corner cases hide all edges and edge cases hide all corners. Dimmed edges are the ignored middle-layer edges.',
        cornerTitle: 'Corner cases',
        edgeTitle: 'Edge cases',
        algTitle: 'Reference algs (UD axis)',
        algCase: 'Edge / Corner',
        algMoves: 'Alg',
        trueFrTitle: 'FR vs Fake FR',
        trueFrBody1: 'Reaching 0 bad edges and the 0 corner shape is not always FR: if the parity of U2/D2 turns is wrong you get Fake FR (shape is met but one more axis half-turn is still needed).',
        trueFrBody2: 'The tool\'s correct FR solution is already parity-fixed; open Solution breakdown on each result card to compare the shape (Fake FR) vs the FR solution.',
      },
      error: {
        parse: 'Cannot parse move: {token} (only U D R L F B with \' / 2 are supported)',
        notHtr: 'The scramble is not in HTR state. Please confirm the cube has completed HTR.',
      },
      axisSuffix: 'axis',
      badEdges: 'Bad edges',
      corner: 'Corner case',
      solution: 'Reference',
      alreadyFr: 'Already FR',
      noSolution: 'Unsolvable',
      highlightTip: 'Switch the axis to observe',
      practice: {
        axisPrompt: 'Choose the FR axis to practice',
        randomAxis: 'Random axis',
        randomAxisHint: '(axis randomly assigned for this scramble)',
        solutionInput: 'Your FR solution',
        solutionPlaceholder: 'e.g. R2 F2 U2',
        liveInvalid: 'Invalid move: {token}',
        liveTrueFr: 'FR reached',
        liveFalseFr: 'FR shape reached, but parity is off (Fake FR)',
        submit: 'Submit',
        next: 'Next',
        resultCorrect: 'FR ✓ — correct!',
        resultWrong: 'Not FR yet. Compare with the reference solution below.',
        resultFalseFr: 'FR shape reached but Fake FR (parity off); one more axis half-turn is needed.',
        yourSolution: 'Your solution',
        historyTitle: 'Practice history',
        historyClear: 'Clear history',
        historyEmpty: 'No practice records yet',
        moves: 'moves',
      },
      tutorial: frPracticeEn,
    },
    pensukeTrainer: {
      title: 'Pensuke Trainer',
      fullName: 'Pensuke Method (Bars Reduction)',
      subtitle: 'Search Leave Slice from HTR with BR and FR track labels',
      crossLinkPensuke: '→ Pensuke Trainer',
      mode: {
        analyze: 'Analyze',
        practice: 'Practice',
        tutorial: 'Tutorial',
      },
      input: {
        scramble: {
          label: 'Scramble',
          placeholder: 'Enter scramble',
        },
        solution: {
          label: 'Solution to HTR',
          placeholder: 'Moves to HTR; () inverse blocks supported',
        },
      },
      btn: {
        random: 'Random',
      },
      frAxis: 'FR axis',
      brAxesHint: 'The other two axes are used automatically as Pensuke BR axes',
      searching: 'Searching shortest LS…',
      referenceSolution: 'Reference',
      alreadyLs: 'Already Leave Slice',
      noSolution: 'No LS solution found',
      start: 'Start',
      help: {
        cases: pensukeHelpEn,
      },
      error: {
        parse: 'Cannot parse: {token}',
        notHtr: 'Input is not HTR.',
      },
      practice: {
        randomAxis: 'Random axis',
        solutionInput: 'Your LS solution',
        solutionPlaceholder: 'e.g. R2 F2 L2',
        liveLs: 'Leave Slice reached',
        liveInvalid: 'Invalid move: {token}',
        submit: 'Submit',
        next: 'Next',
        resultCorrect: 'Leave Slice ✓',
        resultWrong: 'Not Leave Slice',
        historyTitle: 'History',
        historyClear: 'Clear',
        historyEmpty: 'No records',
        moves: 'moves',
      },
      tutorial: pensukePracticeEn,
    },
  },
  wca: {
    competitions: 'WCA Competitions',
    noResults: 'No results available yet.',
    recon: {
      title: 'Reconstructions',
      scramble: 'Scramble',
      scrambleVerified: 'Verified',
      officialScramble: 'Official scramble from WCA',
      submitScrambleOnly: 'Submit scramble only',
      scrambleOnlyHint: 'If this competition has no scramble yet, you can submit just the scramble without a solution.',
      scrambleValid: 'Valid scramble',
      invalidScramble: 'Invalid scramble',
      scrambleMustWrap: 'Scramble must start and end with R\' U\' F',
      description: 'Overall Description',
      descriptionPlaceholder: 'Write an overall description or post-competition summary...',
      saveDescription: 'Save',
      saved: 'Saved',
      submitRecon: 'Submit Reconstruction',
      movesUnit: 'moves',
      participant: 'Participant',
      unofficial: 'Non-participant',
      participantDetected: 'Detected as competition participant',
      nonParticipantDetected: 'Not found in competition results',
      yourOfficialResult: 'Your official result: {moves} moves',
      yourOfficialResultDnf: 'Your official result: DNF',
      viewByScramble: 'By Scramble',
      viewByPerson: 'By Person',
      viewReconstructions: 'View Reconstructions',
      reconCount: '{count} reconstruction(s)',
      noRecons: 'No reconstructions yet. Be the first to submit!',
      noSolutions: 'No solutions submitted yet.',
      resultsPublished: 'Official results have been published by WCA.',
      syncWcaData: 'Sync WCA Data',
      syncingWcaData: 'Syncing...',
      syncQueued: 'Sync job queued.',
      syncFailed: 'Sync failed',
      attempt: 'Attempt {n}',
      progress: '{submitted} / {total} submitted',
      movesMatchOfficial: 'Matches official result',
      movesMismatch: 'Differs from official result',
      latestRecons: 'Latest Reconstructions',
      myRecons: 'My Reconstructions',
      myReconForComp: 'My Reconstruction',
      noMyRecons: 'You haven\'t submitted any reconstructions yet.',
      submissions: '{count} solution(s)',
      sortByLatest: 'Latest',
      sortByCompDate: 'Competition Date',
    },
  },
  comment: {
    title: 'Comments',
    reply: 'Reply',
    delete: 'Delete',
    placeholder: 'Write a comment...',
    replyingTo: 'Replying to',
    mentionTip: 'Type {\'@\'} to mention someone · Ctrl+Enter to submit',
    submit: 'Submit',
    viewAll: 'View all {count} comments',
  },
  notification: {
    title: 'Notifications',
    markAllRead: 'Mark all as read',
    empty: 'No notifications',
    viewAll: 'View all notifications',
    commented: 'commented on your submission',
    replied: 'replied to your comment',
    mentioned: 'mentioned you in a comment',
    liked: 'liked your submission',
    favorited: 'favorited your submission',
    prev: 'Previous',
    next: 'Next',
  },
  stats: {
    title: 'Statistics',
    topLiked: 'Most Liked Solutions',
    topFavorited: 'Most Favorited Solutions',
    topCommented: 'Most Commented Solutions',
    weeklyBestSingles: 'Weekly Best Single',
    weeklyActiveSubmitters: 'Most Active Submitters per Week',
    competitors: 'Competitors',
    topSubmitters: 'Most Active Submitters',
    topWinners: 'Most Wins',
    likes: 'Likes',
    favorites: 'Favorites',
    comments: 'Comments',
    moves: 'Moves',
    submissions: 'Submissions',
    bestSingle: 'Best Single',
    wins: 'Wins',
    noData: 'No data',
    week: 'Week',
    competition: 'Competition',
  },
  point: {
    title: 'Points',
    rankings: 'Rankings',
    rank: 'Rank',
    player: 'Player',
    total: 'Total',
    weekly: 'Weekly',
    daily: 'Daily',
    endlessBoss: 'Boss Challenge',
    endlessRegular: 'Endless',
    league: 'League',
    history: 'Point History',
    source: 'Source',
    points: 'Points',
    competition: 'Competition',
    date: 'Date',
    cumulative: 'Cumulative',
    breakdown: 'Breakdown',
    noData: 'No point data yet. Points are calculated periodically.',
    detail: {
      rank: 'Rank',
      average: 'Average',
      best: 'Best',
      join: 'Participation',
      consecutive: 'Streak',
      bestSingle: 'Best Single',
      bestSingleUnlimited: 'Best Single (Unlimited)',
      levels: 'Levels',
      kickoffs: 'Kickoffs',
      matches: 'Matches',
      wins: 'Wins',
      draws: 'Draws',
    },
  },
  search: {
    title: 'Search',
    placeholder: 'Search users, solutions, scrambles, competitions...',
    quoteHint: 'Use "quotes" for exact phrase matching',
    simple: 'Simple',
    advanced: 'Advanced',
    users: 'Users',
    submissions: 'Solutions',
    scrambles: 'Scrambles',
    competitions: 'Competitions',
    noResults: 'No results found.',
    viewMore: 'View more',
    totalResults: '{total} results',
    sort: 'Sort',
    sortByMoves: 'By moves',
    sortByDate: 'By date',
    sortAsc: 'Ascending',
    sortDesc: 'Descending',
    filters: {
      moves: 'Moves',
      minMoves: 'Min moves',
      maxMoves: 'Max moves',
      startDate: 'Start date',
      endDate: 'End date',
      type: 'Type',
    },
  },
  drTrigger: {
    title: 'DR Trigger Trainer',
    shortTitle: 'DR Trigger',
    description: 'Practice finding DR triggers! You have 10 minutes to solve as many DR triggers as possible.',
    start: 'Start Game',
    playing: 'Playing',
    gameOver: 'Game Over',
    level: 'Level {level}',
    timeRemaining: 'Time Remaining',
    solution: 'Solution',
    submit: 'Submit',
    abandon: 'Abandon Game',
    abandonConfirm: 'Are you sure you want to abandon this game?',
    notDR: 'Solution does not reach DR state',
    optimal: 'Optimal!',
    nearOptimal: 'Near optimal!',
    moves: '{moves} moves',
    optimalMoves: 'Optimal: {moves} moves',
    timeBonus: '+{seconds}s',
    noBonus: 'No time bonus',
    result: {
      title: 'Results',
      levels: 'Levels Completed',
      totalTime: 'Total Time',
      totalBonus: 'Total Time Bonus',
    },
    leaderboard: {
      title: 'Leaderboard',
      highestLevels: 'Highest Levels',
      mostGames: 'Most Games',
      levels: 'Levels',
      games: 'Games',
    },
    stats: {
      title: 'My Stats',
      totalGames: 'Total Games',
      bestLevel: 'Best Level',
      recentGames: 'Recent Games',
    },
    rules: {
      title: 'Rules',
      list: [
        'Each round shows a cube in EO state. Find a solution to reach DR state. EO axis is FB, DR requires UD.',
        'You start with 10 minutes. Solve as many as you can before time runs out.',
        'If your moves ≤ optimal: +10 seconds. If ≤ optimal + 1: +3 seconds. Otherwise no bonus.',
        'Only solutions that reach DR state can be submitted.',
      ],
    },
    history: {
      title: 'Game History',
      detail: 'Game Detail',
      yourSolution: 'Your Solution',
      solutions: 'Solutions',
      optimalSolutions: 'Optimal Solutions',
      showAll: 'Show all ({count})',
      hideSolutions: 'Hide',
      unsolved: 'Last Round (unsolved)',
      columns: {
        index: '#',
        moves: 'Moves',
        rzp: 'RZP',
        arm: 'ARM',
        bonus: 'Bonus',
        time: 'Time',
      },
      filter: {
        optimal: 'Optimal',
        subOptimal: 'Near optimal',
        other: 'Other',
      },
    },
    allCleared: 'All Cleared!',
    allClearedDesc: 'You solved every available trigger! Remaining time: {time}',
    practice: {
      label: 'Practice mode (no ranking)',
      badge: 'Practice',
    },
    mode: {
      label: 'Mode',
      normal: 'Normal',
    },
    rzp: {
      label: 'Select RZP',
    },
    difficulty: {
      label: 'Difficulty (max optimal moves)',
      unlimited: 'Unlimited',
    },
    cases: {
      title: 'DR Trigger Library',
      moves: 'Moves',
      allMoves: 'All',
      rzpFilter: 'RZP Filter',
      n: 'N',
      m: 'M',
      anyRzp: 'Any',
      noResults: 'No triggers found for the selected filters.',
      total: '{total} triggers',
      pairs: 'Pairs',
      arm: 'ARM',
      armFilter: 'ARM Filter',
      solutions: '{count} solutions',
      detail: 'DR Trigger Detail',
      optimalMoves: 'Optimal Moves',
      allSolutions: 'All Solutions',
      eoBreaking: 'EO Breaking',
      include: 'Include',
      only: 'Only',
      merged: 'Merged',
      symmetricVariants: 'Symmetric Variants',
    },
    mergedCases: 'Merged cases',
    showAllCases: 'Show all cases',
    credit: 'RZP case data from',
    signInToPlay: 'Sign in to play',
    resumeGame: 'Resume Game',
    playAgain: 'Play Again',
    noGamesYet: 'No games played yet',
  },
  quiz: {
    title: 'Daily Quiz',
    shortTitle: 'Quiz',
    description: 'Test your FMC knowledge with {count} questions every day!',
    today: 'Today\'s Quiz',
    notStarted: 'Quiz not started yet',
    start: 'Start Quiz',
    submit: 'Submit Answers',
    submitConfirm: 'Are you sure you want to submit? You cannot change your answers afterwards.',
    timeUp: 'Time\'s up!',
    timeRemaining: 'Time Remaining',
    question: 'Q{index}',
    questionType: {
      1: 'Select the best HTR solution for this DR state',
      2: 'Select solutions that can achieve HTR',
      3: 'Select steps that can reduce QT',
      4: 'Select the best solution to leave slice for this HTR state',
    },
    questionTypeNegative: {
      1: ['Select the solution that is ', 'not', ' the best HTR solution'],
      2: ['Select solutions that ', 'cannot', ' achieve HTR'],
      3: ['Select steps that ', 'cannot', ' reduce QT'],
      4: ['Select the solution that is ', 'not', ' the best to leave slice'],
    },
    positivePrompt: 'Select correct answer(s)',
    negativePrompt: 'Select incorrect answer(s)',
    drAxis: 'DR Axis: {axis}',
    lastMove: 'Last Move: {move}',
    result: {
      title: 'Results',
      correct: '{correct}/{total} correct',
      accuracy: 'Accuracy',
      timeLeft: 'Time Left',
      perfect: 'Perfect Score!',
    },
    leaderboard: {
      title: 'Leaderboard',
      accuracy: 'Accuracy',
      time: 'Time Left',
    },
    history: {
      title: 'Past Quizzes',
    },
    review: 'Review Answers',
    reviewTitle: 'Answer Review',
    yourAnswer: 'Your answer',
    correctAnswer: 'Correct answer',
    signInToPlay: 'Sign in to play',
    noQuiz: 'No quiz available for this day',
    expired: 'This quiz has expired. Answers are shown for review only.',
    scramble: 'Scramble',
    multipleChoice: 'Multiple choice — select all that apply',
    viewingUser: 'Viewing {name}\'s answers',
    backToMine: 'Back to my answers',
  },
}
