...
function applyData(data: QuizResponse) {
  ...
  if (data.submission) {
    answers.value = data.submission.answers?.length
      ...
      : Array.from({ length: data.quiz!.questionCount }, () => [])
    ...
    answers.value = Array.from({ length: data.quiz!.questionCount }, () => [])
  }
}
...
<template>
  <div class="container mx-auto px-4 py-6 max-w-3xl">
      ...
      <div class="space-y-6">
        ...
        >
          <div class="flex items-start justify-between mb-2">
            ...
            <div v-if="isFinished" class="shrink-0 ml-2">
              <Icon
          ...
          <div class="mb-3">
            ...
            <Cube3d :moves="q.scramble" :is-static="false" />
          ...
          <div class="grid grid-cols-1 gap-2">
            <div
              ...
              :key="oi"
              class="border-2 p-3 transition-all duration-200 flex items-start gap-2 cursor-pointer"
              :class="getOptionClass(qi, oi)"
              :disabled="isFinished"
              @click="toggleOption(qi, oi)"
            >
              <span class="font-bold text-sm shrink-0 w-6 h-6 flex items-center justify-center border" :class="(answers[qi] || []).includes(oi) ? 'bg-indigo-500 text-white border-indigo-500' : 'border-gray-300 text-gray-400'">
                {{ o.label }}
              </span>
              <span class="font-mono text-sm">{{ o.solution }}</span>
              <span v-if="isFinished && o.correct" class="ml-auto text-green-600 text-xs font-semibold shrink-0">✓</span>
              <span v-if="isFinished && !o.correct && (answers[qi] || []).includes(oi)" class="ml-auto text-red-500 text-xs font-semibold shrink-0">✗</span>
            </div>
          </div>
        </div>
      </div>
    ...
    <!-- Leaderboard -->
    <div v-if="isFinished && leaderboard.length > 0" class="mt-8">
